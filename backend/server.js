...
async function sendComeBackEmails() {
    try {
        ...
        for (const user of users) {
            const { emailSubject, emailContent } = await generateEmailContent(user.user_id); 
...
async function generateEmailContent(userId) {
    try {
        // Query the database for the first piece of content generated by the user
        const result = await pool.query('SELECT data FROM content WHERE user_id = $1 LIMIT 1', [userId]);
        if (result.rows.length === 0) {
            throw new Error('No content found for the user');
        }
        const content = result.rows[0].data;

        // Create the task for generating the email content
        const userInquiry = emailTask.replace('{CONTENT}', JSON.stringify(content));

        // Make the API call to OpenAI to generate the email content
        const chatCompletion = await client.chat.completions.create({
            messages: [
                {
                    role: "system",
        ...
        const parsedResponse = JSON.parse(emailContent);
